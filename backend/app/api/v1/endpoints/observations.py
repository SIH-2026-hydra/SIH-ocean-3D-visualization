...
def get_observations(
    ...
    try:
        records = service.get_observation_records(depth=parsed_depth)
    except (FileNotFoundError, ValueError) as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc

    if not records:
        raise HTTPException(status_code=404, detail='No observation records found for the requested depth.')
